...
public class EstoqueService : IEstoqueService
{
    ...
    public EstoqueService(IMovimentacaoEstoque movimentacaoRepository, ISaldoEstoque saldoRepository, IProduto produtoRepository)
    {
    ...
    {
        // 1. Obter todas as movimentações do produto
        var movimentacoes = _movimentacaoRepository.ListarPorProduto(idProduto);

        // 2. Calcular entradas e saídas
        int entradas = movimentacoes
            .Where(m => m.VUnitario > 0)
            .Sum(m => m.Qtde);

        int saidas = movimentacoes
            .Where(m => m.VUnitario == 0)
            .Sum(m => m.Qtde);

        int saldoQuantidade = entradas - saidas;

        // 3. Calcular custo médio das entradas
        var entradasMov = movimentacoes
            .Where(m => m.VUnitario > 0);

        int totalQuantidadeEntradas = entradasMov.Sum(m => m.Qtde);
        decimal totalValorEntradas = entradasMov.Sum(m => m.VTotal);

        decimal custoMedio = totalQuantidadeEntradas > 0
            ? totalValorEntradas / totalQuantidadeEntradas
            : 0;

        // 4. Calcular valor total do saldo
        decimal valorTotal = saldoQuantidade * custoMedio;

        // 5. Persistir saldo atualizado
        var saldo = new SaldoEstoque(idProduto, DateTime.Now, saldoQuantidade, custoMedio, valorTotal);
