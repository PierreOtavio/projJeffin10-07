...
namespace WindowsFormsApp2.services
{
    ...
    {
        private readonly ConnectClass connectClass;

        public SaldoService(ConnectClass connect)
        {
            this.connectClass = connect;
        }
        ...
        public void AtualizarSaldo(SaldoEstoque saldo)
        {
            ...
            connectClass.InserirGenerico("saldos_estoque", data);
        }

        public SaldoEstoque ObterSaldoAtual(int idProduto)
        {
            using (var conexao = connectClass.ConnectTo())
            {
